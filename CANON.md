# CANON.md — Brassmere / "Fedora Noir": Canonical Direction (FP-01 ruling 2026-08-29)

THIS IS THE SINGLE SOURCE OF TRUTH. Every workstream (Code, Design, art, audio, writing, research) reads from this file; no track invents anything that contradicts it. The 2026-08-29 FP-01 rulings update the prior RTS rewrite with the active integration branch, validated physical audio catalog and hybrid art pipeline. This repo copy supersedes prior session/Drive copies.

Repo: github.com/khuffsphone/legal-crime
ACTIVE integration branch: codex/brassmere-fun-proof-reboot. PROTECTED recovery baseline: rts/isometric-conversion. The old Claude scaffold/default history is not the production trunk.
Build status: FP-01 "First Ten Minutes" is active; showcase presentation is the normal launch and every change must pass the full automated suite plus browser UAT.
...
- Audio: adaptive noir-jazz + brass; sharp firearm pops; escalating sirens. The core 49-file catalog is physically present and validated; expanded atmosphere and final character VO stay gated until their production media is approved.
- Render: HYBRID, fixed 2:1 dimetric. Characters and authored hero props use deterministic 3D-to-sprite renders; the city, lighting, ground and HUD stay procedural/vector. Procedural character art remains the mandatory fallback and QA baseline.

## HYBRID ART PIPELINE (locked for FP-01)
- Meshy/Blender outputs never enter runtime raw. Scale, materials, camera, light, feet anchor, palette and eight-direction framing are normalized before deterministic sprite export.
- Authored sprites must preserve the same footprint, depth anchor and command silhouette as the procedural fallback.
- Missing or invalid authored media degrades to procedural art, never to an invisible unit or broken load.
- Environment breadth stays procedural until one authored asset family proves readable, performant and provenance-complete in the normal build.
- Asset prompts, provider/plan, source, processed output, hashes and runtime keys travel together in a manifest.
...
## UNIT ROSTER (hybrid, period silhouettes)
...
- Named field bodies and the strategic crew roster are one identity. A field DOWN is a permanent casualty in FP-01 (no recovery system exists); mutiny/strategic loss removes the linked body too.
...
## CONTROLS (current)
[E]xtort · [C]ollect · [R]einvest · [G]rease · [K]rew · [L] wire · [H]elp · [T] · [Z] frame-city · [F]ollow · [M]arket (off by default) · [U] vice-upgrade · [O] audio panel · [0] mute · [Space] fast-forward (1×/2×/4×) · [>] skip-week · [P] perf overlay. Offensive: [1] Raid · [2] Sabotage · [3] Assassinate · [4] Lockout · [5] Expand · [6] Recruit. Camera: zoom +/- + wheel, WASD/edge/drag pan, 3 zoom stops (CLOSE/MID/FAR). Debug/QA (DEV-ONLY, inert in a production build): ?debug=turf|mutiny|win|lose · ?arm=1|pistol|shotgun|rifle|hitman|demolitions|all (per-weapon; =all round-robins every tier across the units) · ?scenario=rival-contest|fed-watch|save-roundtrip (deterministic QA boards; NO-X-RAY) · ?skipmenu · ?art=rich/lean · ?market=on/off · ?reveal=1 · ?life=low|med|high. MENU BYPASS: on a COLD load any of ?debug/?arm/?scenario/?skipmenu auto-starts a fresh game past the main menu (once per page load — Quit / Esc-from-endgame returns to the real menu). ENDGAME: on the win/lose newspaper, [Enter]=new game, [Esc]=main menu (a defined exit — never a blank frame).

## CANON DELTAS FOLDED IN THIS REWRITE (the drift that's now corrected)
Bribery channels: 5-with-Army/Savings → FOUR (Beat/Bench/City Hall/Bureau). Heat: "GPO" → federal exposure 50/70/85. Collector: from-minute-one interceptable bottleneck → RE-TIMED (safe early, vulnerable on invasion). Control: spend-to-extort budget → DISTRICT STATUS. Added: the three win paths, the sparse 96² world + districts + fog, the cut Market, hybrid art with procedural fallback, the validated core audio catalog, the camera world/HUD split and the architecture law. "Military" = equipment/specialist upgrades via the four channels (NOT an army).
