...
def frame_camera(cam_obj, basis, bounds, frame_canvas, target_max_h, pad, force_ortho_scale=None):
    ...
    world-unit->pixel scale shared across separate renders so many subjects (e.g. a 25-asset building kit) tile
    at ONE consistent size. The bbox is still used for horizontal centre + foot anchor, just not for the zoom.
    Source-agnostic — this is the single scale-setting call for blockout/fbx/glb. Leave None (default) for the
    character path's per-subject auto-fit. A subject larger than the cell overflows — intended: the shared scale
    is authoritative, not the framing."""
