...
describe('PLAYTEST REGRESSION — the WRAPPER (the path the scene uses) creates the retake act', () => {
  ...
  // pays" and NO act was created (retake silently did nothing). retake.test.ts missed it because the other
  // cases build the act directly via createMoveAndShakedownAct, never through the wrapper.
