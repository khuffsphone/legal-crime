...
import { buildDossierChips, dirtyPercent, type DossierData } from '../src/scenes/hud/dossierStrip';
...
describe('dossier chips — one summary line per drawer', () => {
  const data: DossierData = {
    ...
    pathsDom: 4, pathsTotal: 9, crewIdle: 5, ledgerDirtyPct: 42,
  };
  it('builds five chips in PANELS order, each with its keycap + summary', () => {
    ...
    expect(chips[2].label).toBe('Paths · Dom 4/9');
