...
import {
  synthSamples, buildSynthBuffer, registerSynthSfx, SYNTH_KEYS, HIT_KEYS, STEP_KEYS, DEATH_REACTION_KEYS,
...
describe('synth coverage — every wired hit, footstep, and nonverbal casualty reaction', () => {
  it('SYNTH_KEYS is exactly the six hit keys, four footsteps, and two death reactions', () => {
    ...
    expect(DEATH_REACTION_KEYS).toEqual(['sfx_death_reaction_1', 'sfx_death_reaction_2']);
    expect(SYNTH_KEYS).toEqual([...HIT_KEYS, ...STEP_KEYS, ...DEATH_REACTION_KEYS]);
    expect(new Set(SYNTH_KEYS).size).toBe(12); // all distinct
  });
});
